    ...
    public partial class Form4 : Form
    {
            ...
            if (draggedNode != null)
            {
                ...
                if (targetNode != null)
                {
                    ...
                    if (!mappedRecords.Contains(mapping))
                    {
                        ...
                        listBoxMappedRecords.Items.Add(mapping);
        ...
        private void btnUndo_Click(object sender, EventArgs e)
        {
            if (mappedRecords.Count > 0)
            {
                ...
                listBoxMappedRecords.Items.RemoveAt(listBoxMappedRecords.Items.Count - 1);
        ...
        private void listBoxMappedRecords_SelectedIndexChanged(object sender, EventArgs e)
        {

        }

        private void ListBoxMappedRecords_DoubleClick(object sender, EventArgs e)
        {
            // Remove the selected item from the ListBox and mapped records
            if (listBoxMappedRecords.SelectedItem != null)
            {
                string selectedMapping = listBoxMappedRecords.SelectedItem.ToString();
                mappedRecords.Remove(selectedMapping);
                listBoxMappedRecords.Items.Remove(selectedMapping);

                string[] nodes = selectedMapping.Split(new string[] { " -> " }, StringSplitOptions.None);
                RemoveColorFromNode(nodes[0], txtJsonData1.Nodes);
                RemoveColorFromNode(nodes[1], txtJsonData2.Nodes);
            }
        }


        private void RemoveColorFromNode(string mapping, TreeNodeCollection nodes)
        {
            foreach (TreeNode node in nodes)
            {
                ...
                RemoveColorFromNode(mapping, node.Nodes);
            }
        }
